...
COMPLEX DivideCOMPLEX(COMPLEX C1, COMPLEX C2)
{
    ...
    {
        float hasilReal = (Real(C1) * Real(C2) + Imag(C1) * Imag(C2)) / denominator;
        float hasilImag = (Real(C1) * Real(C2) + Imag(C1) * Imag(C2)) / denominator;

        CreateComplex(&hasil, hasilReal, hasil);
    }
}
...
{
    return fabs(sqrt(Real(C) * Real(C) + Imag(C) * Imag(C)));
}
