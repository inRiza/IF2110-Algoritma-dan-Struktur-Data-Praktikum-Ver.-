...
void ReadCIRCLE(CIRCLE *C)
{
    float x, y, z;
    scanf("%f %f %f\n", &x, &y, &z);
    CreateCIRCLE(C, Absis(Center(*C)), Ordinat(Center(*C)), Radius(*C));
}
/**
...
{
    printf("P(%.0f %.0f)r=%0.f" Absis(Center(C)), Ordinat(Center(C)), Radius(C));
}
/**
 * I.S. : C sembarang
...
{
    float distance = Panjang(Center(C1), Center(C2));
    return (distance == (Radius(C1) + Radius(C2))) || (distance == fabs(Center(C2) - Center(C1)));
}
...
{
    float distance = Panjang(Center(C1), Center(C2));
    return fabs(Center(C2) - Center(C1)) < distance && distance < Center(C2) + Center(C1);
}
